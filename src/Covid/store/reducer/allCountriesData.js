...
const allCountriesData = (state = initialState, action) => {
  switch (action.type) {
    case actionTypes.LOADING_DATA_START:
      return loadingDataStart(state, action);
    case actionTypes.LOADING_DATA_SUCCESS:
      return loadingDataSuccess(state, action);
    case actionTypes.LOADING_DATA_FAIL:
      return loadingDataFail(state, action);
    case actionTypes.SELECT_COUNTRY_KEY:
      return selectCountryKey(state, action);
    case actionTypes.SELECT_COUNTRY_END_DATE_KEY:
      return selectCountryEndDateKey(state, action);
    default:
      return state;
  }
};
