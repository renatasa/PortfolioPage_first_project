...
export class CoffeeShop extends Component {
  ...
  scrollTocoffeeHomeRef = () =>
    window.scrollTo(0, this.coffeeHomeRef.current.offsetTop);
  scrollToexploreOurProductsRef = () =>
    window.scrollTo(0, this.exploreOurProductsRef.current.offsetTop);
  scrollTotodaySpecialRef = () =>
    window.scrollTo(0, this.todaySpecialRef.current.offsetTop);
  ...
  render() {
    ...
    if (
      this.props.loading &&
      this.props.submitOrderError === false &&
      this.props.submitOrderSuccess === false
    ) {
      modalOrLoader = <Loader />;
    } else if (
      !this.props.loading &&
      this.props.submitOrderError &&
      !this.props.submitOrderSuccess
    ) {
      modalOrLoader = (
        <Error
          errorMessage={`Posting order requests is disabled for this demo application. ${this.props.submitOrderError}`}
        />
      );
    } else if (
      !this.props.loading &&
      ...
      allCoffeeShop = (
        <div>
          ...
          <div className="shoppingItems" ref={this.exploreOurProductsRef}>
            <h2 className="heading-2">Explore our products</h2>
            <div className="shoppingItems__allItems">{shoppingItems}</div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    onSubmitOrder: (allItems, totalPrice) =>
      dispatch(actions.submitOrder(allItems, totalPrice)),
    showShoppingSummaryModalAction: () =>
      dispatch(actions.showShoppingSummaryModalAction()),
    closeShoppingSummaryModalAction: () =>
      dispatch(actions.closeShoppingSummaryModalAction()),
  };
};
